(function () {
  ...
  const keywordsElements = document.querySelectorAll('.popular-keywords');

  if (keywordsElements.length > 0) {
    keywordsElements.forEach((keywordsElement) => {
      const dataKeywords = keywordsElement.getAttribute('data-keywords');
      
      if (dataKeywords) {
        const keywordsValue = dataKeywords.split(',');
        ...
        keywordsValue.forEach(function(keyword) {
          ...
          const itemsContainer = keywordsElement.querySelector('.popular-keywords__items');
          if (itemsContainer) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const submitRequestPage = document.querySelector('.submit-request-page');
    const errorPage = document.querySelector('.error-page');
    const newPostPage = document.querySelector('.new-post-page');
    const submitRequest = document.querySelector('.submit-request');

    if (!(submitRequestPage || errorPage || newPostPage) && submitRequest) {
